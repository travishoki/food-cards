...
describe("subCategoriesForTop", () => {
	...
	it("puts entree under main", () => {
		expect(subCategoriesForTop("main").map((i) => i.key)).toContain(
			"entree",
		);
	});

	it("does not put entree under snack", () => {
		expect(subCategoriesForTop("snack").map((i) => i.key)).not.toContain(
			"entree",
		);
	});
	...
	it("puts drinks under drinks", () => {
		expect(subCategoriesForTop("drinks").map((i) => i.key)).toContain(
			"drinks",
		);
	});

	it("puts sides under sides", () => {
		expect(subCategoriesForTop("sides").map((i) => i.key)).toContain(
			"sides",
		);
	});
